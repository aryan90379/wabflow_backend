...
function isAutoGeneratedInfoFollowUpButton(button = {}) {
  const label = String(button.label || button.title || "").trim().toLowerCase();
  return [
    "book now",
    "book appointment",
    "book a visit",
...
export async function continueFlowV2({ flow, business, account, contact, conversation, event }) {
  ...
  for (let i = 0; i < MAX_NODE_STEPS; i += 1) {
    ...
    if (step.type === "message" || step.type === "question") {
      ...
      if (String(step.id || "").startsWith("step_svc_")) {
        buttons = buttons.filter((button) => !isAutoGeneratedInfoFollowUpButton(button));
      } else if (String(step.id || "").startsWith("step_room_list_")) {
        ...
        if (rooms.length > 0) {
          buttons = rooms.map((room) => ({
            id: `room_detail_${room._id}`.slice(0, 48),
            label: (room.name || "").slice(0, 20),
            description: [
              room.price !== null && room.price !== undefined ? `${room.currency || 'INR'} ${room.price}` : '',
              room.description,
